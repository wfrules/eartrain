<template>
    <div>
        <div class="joystick_area" ref="joystick" @dblclick="doSubmit" :style="getJoyStyle()">
            <div v-if="!revealing" class="circle" :style="getHintStyle(poolHint, index)"
                 v-for="(poolHint, index) in pool">
                {{poolHint.caption}}
            </div>
        ...
        <flexbox>
            ...
            <flexbox-item>
                <x-button type="warn" @click.native="toCharts">
                    <svg class="icon" aria-hidden="true" v-if="$store.state.token != ''">
        ...
        <flexbox v-if="false">
            <flexbox-item>
                <x-button type="warn" @click.native='doPlay'>
            ...
            <flexbox-item>
                <x-button type="warn" @click.native='shuffle'>
                    <svg class="icon" aria-hidden="true">
                        <use xlink:href="#icon-zhangjiechuti"></use>
                    </svg>
                </x-button>
            </flexbox-item>
            <flexbox-item v-show="showReveal">
                <x-button type="warn" @click.native='reveal'>
                    <svg class="icon" aria-hidden="true">
            ...
            <flexbox-item>
                <x-button type="warn" @click.native='toggleStandard' v-show="!revealing">
                    <svg class="icon" aria-hidden="true">
...
<script>
    ...
    export default {
        ...
        computed: {
            elapse(){
                return (this.quest_at)?moment(this.now).diff(this.quest_at, 'seconds'):"";
            },
            showReveal() {
                let bRet = false;
                if (!this.revealing) {
                        ...
                        if (objSelf.joystick.touching) {
                            if (objSelf.revealing) {//揭示状态
                                ...
                                if (objSelf.joystick.distance >= TouchLimit) {
                                    ...
                                    if (objSelf.joystick.distance > 5) {//5和limit之间的小范围移动
        ...
        methods: {
            ...
            play(notes) {
                ...
                for (let i = 0; i < notes.length; i++) {
                    _common.play(_common.getValFromParams(notes[i]), {
                        delay: i * dRate,
